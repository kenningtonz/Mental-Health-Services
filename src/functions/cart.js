...
export function addToCart(id, date, time, cost, name) {
    let cartItem = {cartItemID: "", id: id, date: Timestamp.fromDate(date), time: time, cost: cost, name: name}
    ...
    if(currentUser.value.cart === undefined){
        addDoc(collection(db, "carts"), {
            ...
            currentUser.value.cart = {cartID: docRef.id, cartItems: [cartItem], date:  Timestamp.fromDate(new Date(Date.now()))}
