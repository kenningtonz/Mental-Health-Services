    ...
    return (
        <>
            ...
            <section className="redBG textCenter p1 mt-1 mb-1">
                ...
                <p>{receipt.date}</p>
                <p>${receipt.cost}</p>
                <h4 className="bold text1-4 mt1">Services:</h4>
                <div>
                    <ul className="noliststyle">
                        {receipt.services.map((service) => (
                            <li key={service.id}>
                                <p className="text1-2">{service.name} on {service.date} at {getTimeFormat(service.time)}</p>
                            </li>
                        ))}
                    </ul>
                </div>
                <Link to="/"><button className="btn primary mt1" >Return to Home</button></Link>
