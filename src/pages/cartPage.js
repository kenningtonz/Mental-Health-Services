...
const Cart = () => {
  ...
  return (
    <>
      ...
      <section className="redBG textCenter p1 mt-1 mb-1">
        <h1 className='mb2 textCenter flex flex-col gap-05'>     <i className="fa-solid fa-cart-shopping"></i>Your Cart</h1>
        {tempCartItems.value.length == 0 ? <h2>Your cart is empty</h2> : ""}
        {tempCartItems.value.length == 0 ? <Link to="/services"><button className=" btn primary " >Return to Services</button></Link> : (
          <section className="bgWhite card redShadow">
            <CartItemsTable showRemove={true} showLink={true} />
            <Link to="/cart/checkout"><button className=" width-100 btn primary">Checkout</button></Link>
          </section>
        )}
      </section>
